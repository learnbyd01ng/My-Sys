...
/* Return -1, 0, or 1 depending upon whether double *pdItem1 is less
   than, equal to, or greater than double *pdItem2, respectively. */

static int compareDouble(const double *pdItem1, const double *pdItem2)
{
   assert(pdItem1 != NULL);
   assert(pdItem2 != NULL);

   if (*pdItem1 < *pdItem2) return -1;
   if (*pdItem1 > *pdItem2) return 1;
   return 0;
}
...
int main(void)
{
   ...
   /* Sort the array. */
   qsort(pdNumbers, (size_t)iCount, sizeof(double),
      (int(*)(const void*, const void*))compareDouble);
