...
/* The minimum physical length of the array that underlies a
   DynArray object. */
...
static void DynArray_qsort(const void *ppvArray[],
                           int iLeft, int iRight,
                           int (*pfCompare) (const void *pvElement1,
                                             const void *pvElement2))
{
   ...
   if (iRight > iLeft)
   {
      ...
      DynArray_qsort(ppvArray, iLeft, iMid - 1, pfCompare);
      DynArray_qsort(ppvArray, iMid + 1, iRight, pfCompare);
   }
}
                                    ...
                                    const void *pvElement2))
{
   ...
   DynArray_qsort(oDynArray->ppvArray, 0, oDynArray->iLength-1,
                  pfCompare);
